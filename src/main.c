...
void qpe() {
    ...
    for (int i = target_qubit - 1; i >= 0; i--) {
        ...
        // new_gate(cps_gate, 1 << (i + 2), "CPS");
        build_controlled_single_qubit_gate(ps_gate, i, target_qubit, cps_gate);
        // build_controlled_single_qubit_gate(ps_gate, target_qubit - i - 1, target_qubit, cps_gate);
        ...
        // for (int j = target_qubit - i - 1; j <= target_qubit; j++) {
        //     op->qubit_indices[j] = 1;
        // }
...
int main() {
    ...
    // print_qc_amplitude_probabilities_range(qc, 1, 2);
    // print_qc_probabilities(qc);
    // free_qc(qc);
